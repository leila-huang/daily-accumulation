...
const MenuTree: React.FC = ({}) => {
  ...
  const handleClick = () => {
    ...
    const findNode = TreeTools.findNode<MenuDataItem>(menus, (node) => node?.ModelID === modalId)
    console.log('findNode--->', findNode)
    console.timeEnd('findNode')

    console.time('findDeepNode')
    const findDeepNode = TreeTools.findDeepNode<MenuDataItem>(menus, (node) => node?.ModelID === modalId)
    console.log('findDeepNode--->', findDeepNode)

    console.timeEnd('findDeepNode')
  }

  return (<Form layout={'horizontal'} onFinish={handleClick} labelCol={{ span:6 }} style={{width:600,margin:'auto'}}>
    ...
    <FormItem label={'基于原有数据再增加'}>
      <InputNumber style={{width:'180px'}} min={0} value={num} onChange={(e) => setNum(Number(e))}/>
    </FormItem>
    <FormItem label={'查找的ID'}>
      <Input   style={{width:'180px'}} value={modalId} onChange={(e) => setModalId(e.target?.value)}/>
    </FormItem>
